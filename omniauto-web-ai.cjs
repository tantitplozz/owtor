...
class OmniAutoWebAI {
            ...
            res.on("end", () => {
              try {
                ...
                if (tabs.length > 0) {
                  ...
                  this.logger.info(`   WebSocket: ${this.browserWs}`);
                  resolve(true);
                } else {
  ...
  async setupAntiDetection() {
    ...
    this.logger.info(
      `   WebGL Renderer: ${this.currentFingerprint.webgl.renderer.substring(
        0,
        60
      )}...`
    );
...
`);
  ...
  if (initialized) {
    ...
    console.log(
      "📊 Success Rate:",
      result.evaluation?.successRate + "%" || "N/A"
    );

    if (result.evaluation?.recommendations?.length > 0) {
      console.log("\n💡 Recommendations:");
      result.evaluation.recommendations.forEach((rec, i) => {
        console.log(`   ${i + 1}. ${rec}`);
      });
    }

    console.log("\n🎮 Ready for your custom missions!");
    console.log("Provide your mission config to start automation...");
  } else {
